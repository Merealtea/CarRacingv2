...
class Wrapped_Env():
    def __init__(self, env, history_len) -> None:
        self.env = env
        ...
        self.historys = [np.zeros((1, 3, 84, 96))] * 4
    ...
    def reset(self):
        ...
        # Skip initialzation 
        for i in range(40):
            obs, _, _, _, _ = self.env.step(np.array([0,0, 0]))
            
        # # car position
        obs = self.crop_img(obs)
        for i in range(self.history_len):
            # obs, _, _, _, _ = self.env.step(np.array([0,0.2,0]))
            self.add_obs(obs)
        ...
        return np.concatenate(self.historys, axis= 1)
    
    def add_obs(self, o):
        self.historys.append(o)
        self.historys.pop(0)

    
    def crop_img(self, o):
        o = o[:84,:]
        return o[None, ...].transpose((0, 3, 1, 2)) / 255.0
            
    def is_pass(self, x, y):
        _,beta, ckpt_x,ckpt_y = self.env.track[self.next_closet_point]
        ckpt_line = (ckpt_x + math.cos(beta),ckpt_y + math.sin(beta))
